...
<div class="container py-4">
    ...
    <div class="d-flex justify-content-between align-items-center my-4 flex-wrap gap-3">
        ...
        <!-- Recherche -->
        <form action="{{ route('arrivee.index') }}" method="GET" class="d-flex">
            <input
                type="text"
                name="search"
    ...
    <div class="card-style">
        <h4 class="mb-3">📥 Documents d'arrivée</h4>

        @if (session('success'))
            <div class="alert alert-success">{{ session('success') }}</div>
        @endif

        <form method="POST" action="{{ isset($doc) ? route('arrivee.update', $doc->id) : route('arrivee.store') }}" enctype="multipart/form-data" class="row g-3">
            @csrf
            ...
            <div class="col-md-4">
                <input type="text" name="numero_d_envoi" class="form-control" placeholder="N° d'envoi" value="{{ $doc->numero_d_envoi ?? '' }}" required />
            </div>
            ...
            <div class="col-md-12">
                ...
                @if(isset($doc))
                    <a href="{{ route('arrivee.index') }}" class="btn btn-secondary ms-2">Annuler</a>
                @endif
            </div>
        </form>
    </div>
            ...
            <tbody>
                @foreach ($documents as $doc)
                    <tr>
                        ...
                        <td>{{ $doc->date }}</td>
                        <td>{{ $doc->numero_d_envoi }}</td>
                        <td>
                            @if($doc->fichier)
                                <a href="{{ asset('storage/fichiers/' . $doc->fichier) }}" target="_blank">Voir fichier</a>
                            @else
                                Aucun
                            @endif
                        </td>
                        <td>
                            <a href="{{ route('arrivee.edit', $doc->id) }}" class="btn btn-sm btn-warning me-1">✏️</a>
                            <form action="{{ route('arrivee.destroy', $doc->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Confirmer la suppression ?')">
                                @csrf
                                @method('DELETE')
