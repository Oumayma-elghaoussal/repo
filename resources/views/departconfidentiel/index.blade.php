...
<head>
    <meta charset="UTF-8">
    <title>Départ Confidentiel - Documents</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>
        body { background-color: #eef2f7; font-family: 'Segoe UI', sans-serif; }
        .logo-container { text-align: center; margin-top: 30px; margin-bottom: 15px; }
        .logo-container img { max-width: 240px; height: auto; }
        .main-title { text-align: center; font-size: 2.5rem; font-weight: bold; color: #2c3e50; margin-top: 10px; }
        .subtitle { text-align: center; font-size: 1.2rem; color: #7f8c8d; margin-bottom: 30px; }
        .btn-nav { border-radius: 30px; padding: 8px 18px; font-weight: 500; }
        .card-style { background: #fff; border-radius: 12px; padding: 25px; box-shadow: 0 6px 16px rgba(0, 0, 0, 0.06); margin-bottom: 30px; }
...
<div class="container py-4">
    ...
    <div class="subtitle">DAI - Départ Confidentiel</div>
    ...
    <div class="d-flex justify-content-between align-items-center my-4 flex-wrap gap-3">
        <div>
            ...
            <a href="{{ route('arrivee.index') }}" class="btn btn-outline-dark btn-nav me-2">📥 Arrivée Ministérielle</a>
            <a href="{{ route('visa.index') }}" class="btn btn-outline-success btn-nav me-2">📝 Visa</a>
        ...
        <form action="{{ route('departconfidentiel.index') }}" method="GET" class="d-flex">
    ...
    <div class="card-style">
        <h4 class="mb-3">🔒 Documents de départ confidentiel</h4>

        @if (session('success'))
            <div class="alert alert-success">{{ session('success') }}</div>
        @endif

        <form method="POST" action="{{ isset($doc) ? route('departconfidentiel.update', $doc->id) : route('departconfidentiel.store') }}" enctype="multipart/form-data" class="row g-3">
            @csrf
            ...
            <div class="col-md-12">
                ...
                @if(isset($doc))
                    <a href="{{ route('departconfidentiel.index') }}" class="btn btn-secondary ms-2">Annuler</a>
                @endif
            </div>
        </form>
    </div>
            ...
            <tbody>
                @foreach ($documents as $doc)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('departconfidentiel.edit', $doc->id) }}" class="btn btn-sm btn-warning me-1">✏️</a>
                            <form action="{{ route('departconfidentiel.destroy', $doc->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Confirmer la suppression ?')">
                                @csrf
                                @method('DELETE')
