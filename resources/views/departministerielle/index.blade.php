...
<head>
    <meta charset="UTF-8">
    <title>Départ Ministérielle - Documents</title>
...
<div class="container py-4">
    ...
    <div class="subtitle">DAI - Départ Ministérielle</div>
    ...
    <div class="d-flex justify-content-between align-items-center my-4 flex-wrap gap-3">
        <div>
            ...
            <a href="{{ route('departconfidentiel.index') }}" class="btn btn-outline-warning btn-nav me-2">🔒 Départ Confidentiel</a>
            <a href="{{ route('departministerielle.index') }}" class="btn btn-outline-primary btn-nav">🏛️ Départ Ministérielle</a>
        </div>

        <form action="{{ route('departministerielle.index') }}" method="GET" class="d-flex">
    ...
    <div class="card-style">
        <h4 class="mb-3">🏛️ Documents de départ ministérielle</h4>

        @if (session('success'))
            <div class="alert alert-success">{{ session('success') }}</div>
        @endif

        <form method="POST" action="{{ isset($doc) ? route('departministerielle.update', $doc->id) : route('departministerielle.store') }}" enctype="multipart/form-data" class="row g-3">
            @csrf
            ...
            <div class="col-md-12">
                ...
                @if(isset($doc))
                    <a href="{{ route('departministerielle.index') }}" class="btn btn-secondary ms-2">Annuler</a>
                @endif
            </div>
        </form>
    </div>
            ...
            <tbody>
                @foreach ($documents as $doc)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('departministerielle.edit', $doc->id) }}" class="btn btn-sm btn-warning me-1">✏️</a>
                            <form action="{{ route('departministerielle.destroy', $doc->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Confirmer la suppression ?')">
                                @csrf
                                @method('DELETE')
