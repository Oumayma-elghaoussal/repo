...
<div class="container py-4">
    ...
    <div class="subtitle">DAI - depart</div>
    ...
    <div class="d-flex justify-content-between align-items-center my-4 flex-wrap gap-3">
        ...
        <form action="{{ route('depart.index') }}" method="GET" class="d-flex">
    ...
    <div class="card-style">
        <h4 class="mb-3">📤 Documents de départ</h4>

        @if (session('success'))
            <div class="alert alert-success">{{ session('success') }}</div>
        @endif

        <form method="POST" action="{{ isset($doc) ? route('depart.update', $doc->id) : route('depart.store') }}" enctype="multipart/form-data" class="row g-3">
            @csrf
            ...
            <div class="col-md-12">
                ...
                @if(isset($doc))
                    <a href="{{ route('depart.index') }}" class="btn btn-secondary ms-2">Annuler</a>
                @endif
            </div>
        </form>
    </div>
            ...
            <tbody>
                @foreach ($documents as $doc)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('depart.edit', $doc->id) }}" class="btn btn-sm btn-warning me-1">✏️</a>
                            <form action="{{ route('depart.destroy', $doc->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Confirmer la suppression ?')">
                                @csrf
                                @method('DELETE')
